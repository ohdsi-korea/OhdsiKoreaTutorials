...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1322184,40241186)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1322184,40241186)
  and c.invalid_reason is null
...
WHERE V.visit_concept_id in (9203,262,9201,32211,32214,32217,8717,8970,8892,32228,32236,32238,32239,32240,32241,32242,32243,32244,32245,32247,32248,32249,581383,581379,581382,8870,581381)
-- End Condition Occurrence Criteria
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,-28,P.START_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
